...
description: "The aim of this project is to reproduce, layer by layer, a chart on Life Expectancy evolution from Our World in Data."
...
In order to understand such worldwide increase in longevity, we can examine the "Life Expectancy of the World Population in 1800, 1950 and 2012" chart sourced from Our World in Data. Using information on population and life expectancy at birth from Gapminder.org, the mentioned chart illustrates how living standards have increased over the long run through the visualization of life expectancy evolution over the world.

Accordingly, this post aims to replicate the provided chart while proposing potential enhanced and alternative visualizations.

![Source: [Our World in Data](https://ourworldindata.org/life-expectancy-increased-in-all-countries-of-the-world).](life-expectancy.png){.external width="100%"}
...
One particularity of the original plot is that for each of the displayed years, a different group of countries is highlighted. Therefore, specific lists containing the groups of countries to be labelled are created for each of the three years.
...
Before starting to build the graph, we can work on some preliminary steps, that will be applied not only to replicate the chart, but also to enhance and provide alternative versions, allowing us to keep the code cleaner. This way, we create several character vectors containing the caption and the right-side notes to be included later in the plot.
...
After these steps, our data set is ready in order to start creating the plot.

## Create Graph

### Initial elements

To begin, the chart is plotted using geom_line() and geom_area(), enabling the representation of life expectancy in stacked areas for each of the years. To match the plot with the original graph's palette, two vectors are created, one containing the specific colors to fill the areas, and the other to draw the lines.

To adjust the scale of the plot and stick as closely as possible to the shape of the original plot, breaks in the axes are manually introduced along with th axis labels. However, later on, we will need to include some annotations on the right side of the plot. Therefore, the x-limits are set beyond 1 to create additional space.
...
Since the plot involves a large number of labels inside the graph, geom_text_repel() is used to adjust the position of the countries and minimize overlap. Another advantage is that it allows constraining the labels within specified limits, preventing them from extending beyond the graph.
...
One inconsistency I found in the original graph is that the setting of the labels does not follow a fixed criterion, since different groups of countries are labelled for each of the years – some countries appear in the three different years, while other only appear once. Consequently, my first proposal in an enhanced plot based on the replication of the original with an additional small modification, where the same group of countries is highlighted in each line, making the plot more coherent.

To achieve this, a common list for the three countries is created, which will subsequently be used to subset the data when using geom_text_repel. The remaining code is kept as in the replication section.
...
The alternative proposed is an entirely new plot that represents the same information from another perspective. This new proposal aims to simplify the plot, making it more understandable.

Additionally, information regarding the world region to which each country belongs is included, providing a more comprehensive plot and allowing the identification of potential patterns between areas. Also, with this kind of chart, by observing the slopes, we can assess the level of inequality across countries for each year in terms of living standards and life expectancy. For instance, in 1950, we observe a steep slope, meaning that there was a high inequality between countries, despite life expectancy levels were higher than in 1800.

To create this proposal, the chart is plotted using geom_jitter() along with faced_grid(), displaying an independent subplot for each of the years, where countries are represented by the data points.
...
By using geom \_smooth(), a curve is added to the data points, whose slope serves as an indicator of the inequality in terms of life expectancy across countries.
...
plot3 <- plot3 +
  ...
  theme(
    ...
    plot.caption = element_text(size = 7, hjust = 0, vjust = -0.5, color = "#737373"),
    plot.caption.position = "plot",
...
Finally, following the same procedure as in the previous graphs, we create a specific list containing the countries to be highlighted. Subsequently, this information is included in a new column in the data set, allowing us to create a subset when applying geom_text_repel().
...
The final step is to include the annotations indicating the average global life expectancy for each of the years. To place the notes inside each of the subplots, a data frame is created, in which the position along the y- and x-axis is specified. Subsequently, geom_text() is used to include such notes in our final plot.
